...
def number2filename(n):
    return 'note_{0:02d}.tex'.format(n)

def filename2number(s):
    return int(str(s).replace('.tex', '').replace('note_', ''))

class Lecture():
    def __init__(self, file_path, course):
        with file_path.open() as f:
            for line in f:
                lecture_match = re.search(r'note\{(.*?)\}\{(.*?)\}\{(.*)\}', line)
                if lecture_match:
                    break;
    ...
    def __str__(self):
        return f'<Note {self.course.info["short"]} {self.number} "{self.title}">'
...
class Lectures(list):
    ...
    def read_files(self):
        files = self.root.glob('note_*.tex')
    ...
    def new_lecture(self):
        ...
        new_lecture_path.write_text(f'\\note{{{new_lecture_number}}}{{{date}}}{{}}\n')
